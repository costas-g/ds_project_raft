...
local_ip  = localhost#'192.168.2.4' # The LAN IP address of the machine where the cluster runs on. Use this for access over the Internet when port forwarding is set up. 
public_ip = localhost#'192.168.2.11' # The Public IP address for the clients to direct their requests
...
timing = {
    "batching_interval":    time_scale_factor * 0.050,    #0.015
    "heartbeat_interval":   time_scale_factor * 0.250,    #0.075,
...
MAX_LOG = 500          # maximum local log length